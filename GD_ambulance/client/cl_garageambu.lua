...
RMenu.Add('garageambulance', 'main', RageUI.CreateMenu("Garage", "Garage L.S.M.S"))

Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get('garageambulance', 'main'), true, true, true, function() 
        ...
        while true do
            ...
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'ambulance' then    
                    ...
                    if IsControlJustPressed(1,51) then           
                        RageUI.Visible(RMenu:Get('garageambulance', 'main'), not RageUI.Visible(RMenu:Get('garageambulance', 'main')))
                    end   
                end
               end 
        end
...
Citizen.CreateThread(function()
    local hash = GetHashKey("s_m_y_fireman_01")
    while not HasModelLoaded(hash) do
    RequestModel(hash)
    Citizen.Wait(20)
    end
    ped = CreatePed("PED_TYPE_CIVMALE", "s_m_y_fireman_01",  340.68, -582.82, 0.8, 81.85, false, true) --Emplacement du PEDS
